<template>
  <div
    @click="$emit('click')"
    class="bg-white rounded-xl p-3.5 cursor-pointer hover:shadow-md transition-all duration-150 border border-slate-100 group hover:-translate-y-0.5"
  >
    <div class="flex items-center justify-between mb-2.5">
      ...
      <SquarePen
        class="w-3.5 h-3.5 text-slate-300 opacity-0 group-hover:opacity-100 transition"
      />
    </div>
...
<script setup>
import { SquarePen, CalendarDays } from "lucide-vue-next";
